...
// Error values which may be returned by Backoff.
var (
...
func Backoff(function TryFunc, options BackoffOptions) error {
	...
	if options.Sleeper == nil {
		options.Sleeper = libtime.NewSleeper()
	}
	...
	for consumed < options.MaxSleepTime {
		...
		if err != nil && !keepRetrying {
			return errors.Wrap(err, "exponential backoff instructed to stop retrying")
		}
		...
		options.Sleeper.Sleep(duration)
